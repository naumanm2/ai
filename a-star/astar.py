...
def astar(start_state, goaltest, h):
    ...
    while not Q.empty():
        ...
        prio, current_state = Q.get()
        if (goaltest(current_state)):
            path = []
            while current_state is not start_state:
                path.append(predecessor[current_state][1])
                current_state = predecessor[current_state][0]
            return path[::-1]
            
        for aname, s in current_state.successors():
            if s in g:
                if (g[current_state]+aname.cost < g[s]):
                    predecessor[s] = (current_state, aname)
                    g[s] = g[current_state]+aname.cost
                    Q.put( (h(s)+g[s], s) )
            else:
                g[s] = g[current_state]+aname.cost
                predecessor[s] = (current_state, aname)
